...
/* ARENDASKLADA_MOBILE_AREA_CARD_V60_START */
(()=>{const mobile=()=>matchMedia('(max-width:820px)').matches;let active=null,place=null;const restore=()=>{if(active instanceof HTMLElement&&place&&place.parentNode)place.replaceWith(active);active=null;place=null};
addEventListener('arendasklada:map-object-select',e=>{if(!mobile())return;restore();const r=document.querySelector('[data-property-cards]'),o=document.querySelector('.mobile-map-card-popover'),l=o?.querySelector('.mobile-map-card-popover__slot'),order=String(e.detail?.order||''),c=r instanceof HTMLElement?[...r.querySelectorAll('[data-property-card]')].find(x=>String(x.getAttribute('data-order')||'')===order):null;if(!(c instanceof HTMLElement)||!(l instanceof HTMLElement)||!(o instanceof HTMLElement))return;
place=document.createComment('mobile-map-card');c.before(place);active=c;c.hidden=false;c.classList.remove('mobile-page-hidden','is-map-hovered');l.replaceChildren(c);o.hidden=false;document.body.classList.add('mobile-map-card-open')});document.addEventListener('click',e=>{const t=e.target;if(!(t instanceof Element))return;if(t.closest('.mobile-map-card-popover__close')||t.matches('.mobile-map-card-popover'))restore()},true);
const lock=node=>{if(!(node instanceof HTMLElement)||node.dataset.mobileAreaLock)return;const s=node.__arendaYandexMap;if(s?.map&&s?.objectManager){const objects=s.objectManager.getBounds?.();if(!objects)return;node.dataset.mobileAreaLock='wait';s.map.setBounds(objects,{checkZoomRange:true,zoomMargin:[70,70,110,70]});
setTimeout(()=>{const area=s.map.getBounds();s.map.options.set('restrictMapArea',area);node.dataset.mobileAreaLock='1'},250)}else if(s?.leafletMap&&s?.leafletMarkers){const points=s.leafletMarkers.map(x=>x.marker.getLatLng());if(!points.length)return;node.dataset.mobileAreaLock='wait';s.leafletMap.fitBounds(points,{padding:[46,46],maxZoom:13});
setTimeout(()=>{const area=s.leafletMap.getBounds();s.leafletMap.setMaxBounds(area);s.leafletMap.options.maxBoundsViscosity=1;node.dataset.mobileAreaLock='1'},100)}};const scan=()=>{if(mobile())document.querySelectorAll('#warehouses [data-yandex-map]').forEach(lock)};scan();[300,900,1800].forEach(x=>setTimeout(scan,x));addEventListener('resize',scan);new MutationObserver(scan).observe(document.body,{childList:true,subtree:true})})();
/* ARENDASKLADA_MOBILE_AREA_CARD_V60_END */

/* ARENDASKLADA_MOBILE_TIGHT_MAP_V62_START */
(()=>{const mobile=()=>matchMedia("(max-width:820px)").matches;
const fit=node=>{const s=node.__arendaYandexMap;if(s?.map&&s?.objectManager){const b=s.objectManager.getBounds?.();if(!b)return false;s.map.setBounds(b,{checkZoomRange:true,zoomMargin:[16,16,38,16]});setTimeout(()=>s.map.options.set("restrictMapArea",s.map.getBounds()),220);return true}
if(s?.leafletMap&&s?.leafletMarkers){const p=s.leafletMarkers.map(x=>x.marker.getLatLng());if(!p.length)return false;s.leafletMap.fitBounds(p,{padding:[16,16],maxZoom:14});setTimeout(()=>{const b=s.leafletMap.getBounds();s.leafletMap.setMaxBounds(b);s.leafletMap.options.maxBoundsViscosity=1},120);return true}return false};
const bind=()=>{if(!mobile())return;document.querySelectorAll("#warehouses .catalog-map-panel").forEach(panel=>{const node=panel.querySelector("[data-yandex-map]"),box=panel.querySelector(".map-use-checkbox");if(!(node instanceof HTMLElement)||!(box instanceof HTMLInputElement))return;
if(node.dataset.tightBounds!=="1"&&fit(node))node.dataset.tightBounds="1";if(box.dataset.cancelReset==="1")return;box.dataset.cancelReset="1";box.addEventListener("change",()=>{if(!box.checked)fit(node)})})};
bind();[300,700,1300,2200].forEach(ms=>setTimeout(bind,ms));new MutationObserver(bind).observe(document.body,{childList:true,subtree:true})})();
/* ARENDASKLADA_MOBILE_TIGHT_MAP_V62_END */
