---
published: true
order: 9
pageSlug: logisticheskiy-centr-panelniy
block: Склад у ж/д ветки, Яшнабадский район
floor: "1"
area: 5000
workplaces: '6 м'
ready: 'Действующий'
warehouseLayout: 'Сухая аренда'
title: 'Логистический центр — Панельный'
detailTitle: 'Логистический центр — Панельный · 5000 м² в Ташкенте'
seoTitle: 'Логистический центр — Панельный — склад 5000 м² в аренду в Ташкенте'
description: 'Склад расположен рядом с железнодорожной веткой. Часть складских помещений является таможенными помещениями и оснащена системами поддержания температуры до +25°C.'
imageAlt: 'Иллюстрация без фотографии: Логистический центр — Панельный'
intro: 'Склад расположен рядом с железнодорожной веткой. Часть складских помещений является таможенными помещениями и оснащена системами поддержания температуры до +25°C.'
features:
- '5 000 м²'
- 'Яшнабадский район'
- '+15 - +25 C'
- 'Сухая аренда'
- 'Действующий'
detailFeatures:
- 'Площадь: 5 000 м²'
- 'Район/локация: Яшнабадский район'
- 'Тип: Склад у ж/д ветки'
- 'Высота потолков: 6 м'
- 'Температурный режим: до +25°C'
- 'Тип аренды: Сухая аренда'
- 'Готовность: Действующий'
- 'Рядом с ж/д веткой'
- 'Таможенные помещения'
- 'Температура до +25°C'
- 'Высота 6 м'
planImage: '/images/objects/object-9-plan.svg'
mainImage: "/images/objects/object-1-main.svg"
gallery: ["/images/objects/object-9-plan.svg"]
presentationFile: '/files/placeholder-presentation-1.pdf'
region: 'Ташкент'
city: 'Ташкент'
district: 'Яшнабадский район'
address: 'Яшнабадский район'
propertyType: 'Склад у ж/д ветки'
warehouseClass: 'B'
ceilingHeight: '6 м'
dockDoors: 'по запросу'
temperature: 'до +25°C'
temperatureMode: '+15 - +25 C'
leaseType: 'Сухая аренда'
builtYear: 1972
price: 'По запросу'
availableNow: true
mapX: 70
mapY: 48
latitude: 41.3
longitude: 69.345
---
