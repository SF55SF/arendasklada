---
published: true
order: 1
pageSlug: logisticheskiy-centr-urta-mahalla
block: Склад, махаллинский сход граждан Урта
floor: "1"
area: 6000
workplaces: 'по запросу'
ready: 'Существующие'
warehouseLayout: '"Сухая" аренда'
title: 'Логистический центр — Урта Махалла'
detailTitle: 'Логистический центр — Урта Махалла · 6000 м² в Ташкенте'
seoTitle: 'Логистический центр — Урта Махалла — склад 6000 м² в аренду в Ташкенте'
description: 'Склад оборудован для напольного хранения и сейчас работает без установленных стеллажей.'
imageAlt: 'Иллюстрация без фотографии: Логистический центр — Урта Махалла'
intro: 'Склад оборудован для напольного хранения и сейчас работает без установленных стеллажей.'
features:
- '6 000 м²'
- 'махаллинский сход граждан Урта'
- 'Естественное'
- '"Сухая" аренда'
- 'Существующие'
detailFeatures:
- 'Площадь: 6 000 м²'
- 'Район/локация: махаллинский сход граждан Урта'
- 'Тип: Склад'
- 'Высота потолков: по запросу'
- 'Температурный режим: +15 - +25 C'
- 'Тип аренды: "Сухая" аренда'
- 'Готовность: Существующие'
- 'Напольное хранение'
- '1 этаж'
- 'Площадь 6000 м²'
- 'Год постройки 2020'
planImage: '/images/objects/object-1-plan.svg'
mainImage: '/images/objects/object-1-main.svg'
gallery:
- '/images/objects/object-1-main.svg'
- '/images/objects/object-1-plan.svg'
- '/images/objects/object-1-scheme.svg'
presentationFile: '/files/placeholder-presentation-1.pdf'
region: 'Ташкент'
city: 'Ташкент'
district: 'махаллинский сход граждан Урта'
address: 'махаллинский сход граждан Урта'
propertyType: 'Склад'
warehouseClass: 'B'
ceilingHeight: 'по запросу'
dockDoors: 'по запросу'
temperature: '+15 - +25 C'
temperatureMode: 'Естественное'
leaseType: '"Сухая" аренда'
builtYear: 2020
price: '$ 9 за м², НДС не включен'
availableNow: true
mapX: 38
mapY: 48
latitude: 41.286
longitude: 69.155
---
