---
published: true
order: 7
pageSlug: logisticheskiy-centr-risoviy
block: Холодный склад, Мирабадский район
floor: "1"
area: 5000
workplaces: '6 м'
ready: "Действующий"
warehouseLayout: 'Стеллажная аренда'
title: 'Логистический центр — Рисовый'
detailTitle: "Логистический центр — Рисовый · 5000 м² в Ташкенте"
seoTitle: 'Логистический центр — Рисовый — склад 5000 м² в аренду в Ташкенте'
description: 'Логистический комплекс для хранения товаров с охлаждением. Помещения поддерживают температуру от -20°C до +4°C, высота хранения составляет 6 м, склад оснащен WMS.'
imageAlt: 'Иллюстрация без фотографии: Логистический центр — Рисовый'
intro: 'Логистический комплекс для хранения товаров с охлаждением. Помещения поддерживают температуру от -20°C до +4°C, высота хранения составляет 6 м, склад оснащен WMS.'
features:
- '5 000 м²'
- 'Мирабадский район'
- 'Холодильник'
- 'Стеллажная аренда'
- 'Действующий'
detailFeatures:
- 'Площадь: 5 000 м²'
- 'Район/локация: Мирабадский район'
- 'Тип: Холодный склад'
- 'Высота потолков: 6 м'
- 'Температурный режим: -20°C до +4°C'
- 'Тип аренды: Стеллажная аренда'
- 'Готовность: Действующий'
- 'Температура -20°C до +4°C'
- 'Высота хранения 6 м'
- 'WMS'
- '1 этаж'
planImage: "/images/objects/object-7-plan.svg"
mainImage: "/images/objects/object-1-main.svg"
gallery: ["/images/objects/object-7-plan.svg","/images/objects/object-7-scheme.svg"]
presentationFile: '/files/placeholder-presentation-1.pdf'
region: 'Ташкент'
city: 'Ташкент'
district: 'Мирабадский район'
address: "Мирабадский район"
propertyType: 'Холодный склад'
warehouseClass: "B"
ceilingHeight: "6 м"
dockDoors: "по запросу"
temperature: '-20°C до +4°C'
temperatureMode: "Холодильник"
leaseType: "Стеллажная аренда"
builtYear: 2000
price: "По запросу"
availableNow: true
mapX: 58
mapY: 57
latitude: 41.285
longitude: 69.295
---
